...
class BridgeController {
	executeBridge = async (req: Request, res: Response) => {
		...
		try {
			const routes = await bridges.bestBridge(
	...
	test = async (req: Request, res: Response) => {
		let data: any = await fetch(
			"http://localhost:5000/api/bridge/execute-best-route?fromChainId=1&toChainId=137&fromTokenAddress=0x7d1afa7b718fb893db30a3abc0cfc608aacfebb0&toTokenAddress=0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174&amount=100000000000000000000&receiver=0x4e7f624C9f2dbc3bcf97D03E765142Dd46fe1C46"
		);
	...
	bestBridge = async (req: Request, res: Response) => {
		...
		try {
			var routes = await bridges.bestBridgeV2(
	...
	bestBridgeV2 = async (req: Request, res: Response) => {
		...
		const routes = await bridges.bestBridgeV2(
			ChainId.POL,
			ChainId.ETH,
			CoinKey.USDC,
			CoinKey.USDC,
