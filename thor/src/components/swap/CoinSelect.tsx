      ...
      <div className="relative w-2/5">
        <div
          ...
          onClick={(e) => {
            ...
            setIsDropDownOpenCoin(!isDropDownOpenCoin)}}
        >
          <div className="flex h-12 w-full cursor-pointer select-none flex-row justify-between bg-[#464646] px-1 text-white">
        ...
        {isDropDownOpenCoin && (
          <div className="absolute top-12 left-0 z-10 w-full overflow-hidden rounded-b-md bg-[#464646] mt-2 text-white shadow">
            {supportedCoins.map((coin: Token) => (
              <div
                key={coin.chainAgnositcId}
                className="flex h-11 w-full cursor-pointer select-none flex-row justify-between bg-[#464646] mt-2 py-2.5 pl-3 pr-2 text-white hover:bg-gray-900"
                onClick={(e) => {
                  e.stopPropagation()
                  selectedCoin(coin)}}
              >
