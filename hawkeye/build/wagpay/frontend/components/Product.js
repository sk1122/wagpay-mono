...
const Product = (props) => {
    ...
    const [noProducts, setNoProducts] = (0, react_1.useState)(0);
    const handleCheckChange = (changed) => {
        if (changed) {
            console.log('1');
    ...
    return (<li className="flex py-6 sm:py-10">
			...
			<div className="relative pr-9 sm:grid sm:grid-cols-2 sm:gap-x-6 sm:pr-0">
				...
				<div className="flex justify-between">
					...
					<a className="font-jakarta font-black text-2xl text-gray-200 hover:text-gray-800">
						{props.product.name}
					</a>
					</h3>
				</div>
				...
				<div className={(props.selectProducts ? "hidden " : "") + "mt-4 sm:mt-0 sm:pr-9"}>
					<div className="flex justify-center items-center space-x-2 absolute top-0 right-0">
						...
						<solid_1.PlusIcon onClick={(e) => handleCheckChange(true)} className="h-5 w-5" aria-hidden="true"/>
						</button>
						<p className="text-white">{noProducts}</p>
						<button type="button" className="-m-2 inline-flex p-2 text-gray-400 hover:text-gray-500">
						<span className="sr-only">Minus</span>
						<solid_1.MinusIcon onClick={(e) => handleCheckChange(false)} className="h-5 w-5" aria-hidden="true"/>
						</button>
					</div>
				</div>
			</div>
			...
			<p className="mt-4 flex flex-col space-y-2 text-sm text-gray-200">
				...
				<div className="flex flex-col space-y-3">
					{props.product.links.map(value => (<a href={value} className="text-xs white underline">{value}</a>))}
		 		</div>
			</p>
			</div>
		</li>
